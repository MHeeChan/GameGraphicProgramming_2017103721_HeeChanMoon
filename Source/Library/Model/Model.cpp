...
namespace library
{
    ...
    void Model::Update(_In_ FLOAT deltaTime)
    {
        ...
        if (m_pScene->HasAnimations())
        {
            ...
            FLOAT Ticks = static_cast<FLOAT>(m_pScene->mAnimations[0]->mTicksPerSecond != 0.0f ? m_pScene->mAnimations[0]->mTicksPerSecond : 25.0f);
            FLOAT Time = m_timeSinceLoaded * Ticks;
            FLOAT TimeTicks = fmod(Time, static_cast<FLOAT>(m_pScene->mAnimations[0]->mDuration));

            if (m_pScene->mRootNode)
            {
                readNodeHierarchy(TimeTicks, m_pScene->mRootNode, matrix);
